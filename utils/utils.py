...
def stereo_validation(net, val_data_loader, save_dir=None, device=None):
    psnr_list = []
    ssim_list = []
    psnr_list2 = []
    ssim_list2 = []
    ...
    for batch_id, data in enumerate(tbar):
        with torch.no_grad():
            ...
            haze_all = torch.cat((haze, haze2), dim=1)

            dehaze = net(haze_all)
            dehaze, dehaze2 = dehaze[:,0:3,:,:], dehaze[:,3:,:,:]
        ...
        tbar.set_description("===> Test({}/{}): Error(psnr, ssim): ({:.4f}, {:.4f})".format(batch_id, len(val_data_loader), avg_psnr, avg_ssim))
